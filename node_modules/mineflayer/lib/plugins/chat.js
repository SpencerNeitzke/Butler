...
function inject(bot) {
  bot.client.on(0x03, function(packet) {
    ...
    if(jsonMsg.translate == 'chat.type.text' || jsonMsg.translate == 'commands.message.display.incoming') {
	    var username = jsonMsg.using[0];
	    var content = jsonMsg.using[1];
	    var legalContent = content.replace(incomingFilter, '');
	    bot.emit('message', legalContent, content);
	    if(jsonMsg.translate == 'chat.type.text') {
		    // spoken chat
		    bot.emit('chat', username, content, null);
		} else if (jsonMsg.translate == 'commands.message.display.incoming') {
			// whispered chat
			bot.emit('whisper', username, content, null);
		}
	}
  });
